...
class RedirectTable extends React.Component {
	...
	exportRedirects() {
		this.setState(
			...
			() => {
				RequestUtil.post('wds_export_redirects', nonce)
					.then((data) => {
	...
	importRedirects(file) {
		this.setState({ loading: true }, () => {
			RequestUtil.uploadFile('wds_import_redirects', nonce, file)
				.then((data) => {
	...
	handleSave() {
		...
		this.ajaxPost('wds_save_redirect', data)
			.then((resp) => {
				this.setState(
					{
	...
	handleBulkUpdate() {
		...
		this.ajaxPost('wds_bulk_update_redirects', data)
			.then((resp) => {
				const spec = {};
				ids.forEach((id) => {
					spec[id] = { $set: { ...resp[id] } };
	...
	handleDelete() {
		...
		this.setState({ loading: true });
		this.ajaxPost('wds_delete_redirect', { ids })
			.then(() => {
	...
	handleDeleteAll() {
		...
		this.ajaxPost('wds_delete_redirect')
			.then(() => {
				this.setState(
					{
						redirects: {},
