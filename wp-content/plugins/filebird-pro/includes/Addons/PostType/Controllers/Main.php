...
class Main {
    ...
    public function restSetSetting( \WP_REST_Request $request ) {
        ...
        $config   = Init::getInstance()->load_config( 'post', $user_id );
    ...
    private static function getCountFromTerms( $post_type, $lang = null ) {
        ...
        $join = $wpdb->prepare(
            ...
            INNER JOIN $wpdb->posts as posts ON posts.ID = term_relationships.object_id AND (posts.post_status = 'publish' OR posts.post_status = 'draft')
        ...
        $clauses = apply_filters( 'fbv_post_type_term_counter', $join, $where, $groupby, $lang );
        ...
        $query = "SELECT terms.term_id, COUNT(term_relationships.object_id) as counter 
    ...
    private static function getCountOfPost( $post_type, $lang ) {
       ...
       $query = $wpdb->prepare( "SELECT COUNT(ID) FROM $wpdb->posts WHERE post_type = %s AND (post_status = 'publish' OR post_status = 'draft')", $post_type );
